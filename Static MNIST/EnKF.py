...
def EnKF(X,Y,X0,A,h,t,tau,Noise,h_index,r):
    X = X.to('cpu')
    Y = Y.to('cpu')
    A = A.to('cpu')
...
# =============================================================================
        ...
        for i in range(N):
# =============================================================================
#             X_enkf_torch = torch.from_numpy(x_EnKF[i,]).to(torch.float32)
# =============================================================================
            X_enkf_torch = torch.from_numpy(x_EnKF[i,]+sigmma*np.random.randn(L,J)).to(torch.float32)
# =============================================================================
#             z = Variable(torch.randn(J, 100))
# =============================================================================
            Z = A(X_enkf_torch.T).to('cpu').detach().reshape(-1,28,28)

...
def OT(X,Y,X0_const,parameters,G,A,h,t,tau,Noise,h_index,r):
...
# =============================================================================
    ...
    class NeuralNet(nn.Module):
        ...
        def __init__(self, input_dim, hidden_dim,r):
...
# =============================================================================
#             self.layer_input = nn.Linear(self.input_dim[0]+10, self.hidden_dim, bias=True)
# =============================================================================
            ...
            self.encoder = nn.Sequential(
                ...
                nn.ReLU(True),
                # 8 x (r-4) x (r-4) = 200 for r =9
                nn.Flatten(),
                nn.Linear(8 * (self.r-4) * (self.r-4), 10),
                # 10
                nn.Softmax(dim=1),
                )
...
# =============================================================================
#             y = self.encoder(y.reshape(-1,1,self.r,self.r))
# =============================================================================
...
#                 
#                 z = G(x.to(device)).to('cpu').detach().reshape(-1,28,28)
#                 o_hat = (h(z,ind).T).mean(axis=1)
#                 a = (x - m_hat)
#                 b = (h(z,ind) - o_hat)
...
# =============================================================================
    ...
    def train(f,T,X_Train,Y_Train, iterations,learning_rate,ts,Ts,batch_size,k,K,ind):
        f.train()
        T.train()
        optimizer_T = torch.optim.Adam(T.parameters(), lr=learning_rate/1)
        optimizer_f = torch.optim.Adam(f.parameters(), lr=learning_rate/1)
# =============================================================================
#         optimizer_T = torch.optim.Adam(T.parameters(), lr=learning_rate/1, betas=(0.9,0.999))
#         optimizer_f = torch.optim.Adam(f.parameters(), lr=learning_rate/1, betas=(0.9,0.999))
# =============================================================================
# =============================================================================
...
# =============================================================================
        ...
        for i in range(iterations):
            ...
            for j in range(inner_iterations):
                map_T = T.forward(X_train,Y_shuffled,ind)
...
# =============================================================================
            ...
            if  (i+1)==iterations or i%100==0:
                with torch.no_grad():
                    f_of_xy = f.forward(X_Train,Y_Train) 
                    map_T = T.forward(X_Train,Y_Train_shuffled,ind)
    ...
    # =============================================================================
                    ...
                    #print(g.W.data)
                    print("Simu#%d/%d ,Time Step:%d/%d, Iteration: %d/%d, loss = %.4f" %(k+1,K,ts,Ts,i+1,iterations,loss.item()))
...
# =============================================================================
    ...
    SAVE_all_X_OT = np.zeros((AVG_SIM,N+1,SAMPLE_SIZE,L))
    # =============================================================================
...
# =============================================================================
    ...
    # =============================================================================
        ...
        convex_f = NeuralNet(INPUT_DIM, NUM_NEURON,r)
        MAP_T = T_NeuralNet(INPUT_DIM, NUM_NEURON,r)
...
# =============================================================================
#             X0 = abs(X0 + np.random.multivariate_normal(np.zeros(L),sigmma*sigmma * np.eye(L),SAMPLE_SIZE))
#             X0 = X0/X0.sum(axis=1).reshape(-1,1)
# =============================================================================
# =============================================================================
#             X0 = torch.from_numpy(X0)
# =============================================================================
            X0 = A(X0)
            X0 = torch.from_numpy(X0+sigmma*np.random.randn(J,L))
...
# =============================================================================
            Z = G(X0.to(device)).to('cpu').detach().reshape(-1,28,28)
            
# =============================================================================
#             Z = A(torch.tensor(X0).to(torch.float32).to(device) ) 
# =============================================================================
            
            eta_train = np.random.multivariate_normal(np.zeros(dy),gamma*gamma * np.eye(dy),SAMPLE_SIZE)
            
            Y1 = np.array(h(Z.to('cpu').detach(),h_index[i]) + eta_train)
# =============================================================================
#             print( h(X1.T))
# =============================================================================
...
# =============================================================================
            ...
            train(convex_f,MAP_T,X0,Y1,ITERS,LR,i+1,N,BATCH_SIZE,k,AVG_SIM,h_index[i])
...
# =============================================================================
            ...
            X1_test = X0.to(torch.float32).to(device)
            Y1_true = (Y1_true*torch.ones((X1_test.shape[0],dy))).to(device)
...
# =============================================================================
            ...
            map_T = MAP_T.forward(X1_test, Y1_true,h_index[i])
...
# =============================================================================
            ...
            x_OT[i+1,:] = (torch.mean(map_T.to('cpu'),dim=0)).detach().numpy()
            SAVE_all_X_OT[k,i+1,:,:] = map_T.to('cpu').detach().numpy()
...
# =============================================================================
#             mse_OT[:,k] =  ((x - x_OT)*(x - x_OT)).mean(axis=1).to('cpu')
# =============================================================================

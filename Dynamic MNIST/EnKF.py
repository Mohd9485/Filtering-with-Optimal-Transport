...
def EnKF(X,Y,X0,G,A,h,t,tau,Noise,h_index,r):
    X = X.to('cpu')
    Y = Y.to('cpu')
    G = G.to('cpu')
    ...
    SAVE_X_EnKF =  np.zeros((AVG_SIM,N+1,L,J))
# =============================================================================
...
# =============================================================================
        ...
        for i in range(N):
            X_enkf_torch = torch.from_numpy(A(x_EnKF[i,])).to(torch.float32)
# =============================================================================
#             print(X_enkf_torch.shape)
# =============================================================================
            X_enkf_torch =  X_enkf_torch + torch.from_numpy(sigmma*np.random.randn(L,J)).to(torch.float32)
# =============================================================================
#             z = Variable(torch.randn(J, 100))
# =============================================================================
            Z = G(X_enkf_torch.T).to('cpu').detach().reshape(-1,28,28)

# =============================================================================
#             Z =  A(X_enkf_torch.to('cpu').T)
# =============================================================================
            ...
            x_EnKF[i+1,:,:] = x_hatEnKF + np.matmul( K_EnKF, (y[i,:] - y_hatEnKF).transpose())

            x_hatEnKF = x_EnKF[i+1,:,:]

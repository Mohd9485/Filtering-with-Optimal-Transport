...
def OT(X,Y,X0_const,parameters,A,h,t,tau,Noise):
    Odeint = False
    AVG_SIM = X.shape[0]
    N = X.shape[1]
    L = X.shape[2]
...
# =============================================================================
    ...
    def train(f,T,X_Train,Y_Train, iterations,learning_rate,ts,Ts,batch_size,k,K):
        f.train()
        T.train()
        optimizer_T = torch.optim.Adam(T.parameters(), lr=learning_rate/(10*5)) 
...
# =============================================================================
    print("--- OT time : %s seconds ---" % (time.time() - total_time))
    return SAVE_all_X_OT,run_time/AVG_SIM 
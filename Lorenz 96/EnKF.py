...
def EnKF(X,Y,X0,A,h,t,tau,Noise,rk45):
    AVG_SIM = X.shape[0]
    N = X.shape[1]
    L = X.shape[2]
    dy = Y.shape[2]
...
# =============================================================================
        ...
        for i in range(N-1):
            ...
            if rk45:
# =============================================================================
#                 x_hatEnKF = (odeint(A, x_EnKF[i,].reshape(-1), t[i:i+2])[1,]).reshape(L,J) + sai_EnKF
# =============================================================================

                solver =  RK45(A, t[i], x_EnKF[i,].reshape(-1),T,first_step=tau) 
                solver.step()
                x_hatEnKF = solver.y.reshape(L,J) + sai_EnKF
            else: 
                x_hatEnKF = x_EnKF[i,]+ A(x_EnKF[i,],t[i])*tau  + sai_EnKF
